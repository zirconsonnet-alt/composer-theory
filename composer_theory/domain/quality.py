...
class Quality(FrozenSlotsMixin, metaclass=InternedMeta):
    ...
    @staticmethod
    def _tension_token(iv: Intervals) -> str:
        deg, semi = iv.value
        d = deg.value
        if d not in (2, 4, 6):
            return iv.name
        base_semi = {2: 2, 4: 5, 6: 9}[d]
        delta = (semi - base_semi + 6) % 12 - 6
        if delta == 0:
            return f"add{d}"
        if delta < 0:
            return "b" * (-delta) + str(d)
        return "#" * delta + str(d)
    ...
    def name(self) -> str:
        ...
        if self.tensions:
            ...
            s += "_" + "_".join(self._tension_token(iv) for iv in ts)
        if self.omits:
    ...
    def composition(self) -> FrozenSet[Degrees]:
        degrees = {Degrees.I}
        for iv in self.base.value:
            degrees.add(iv.degree)
        for iv in self.tensions:
            degrees.add(iv.degree)
        for iv in self.omits:
